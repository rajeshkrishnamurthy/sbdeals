...
func main() {
	...
	clickedStore := clicked.NewPostgresStore(sqlDB)
	customerStore := customers.NewPostgresStore(sqlDB)
	server := web.NewServerWithAllStores(supplierStore, bookStore, bundleStore, railStore, clickedStore, customerStore)
