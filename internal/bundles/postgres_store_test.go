...
func TestPostgresStoreListAndListBooksForPicker(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(listQuery)).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "supplier_name", "category", "allowed_conditions", "bundle_price", "book_count"}).
			AddRow(1, "Starter", "A1", "Fiction", "Very good||Good as new", 499.0, 2))

	pickerQuery := `SELECT id, title, author, supplier_id, category, condition, mrp, my_price, bundle_price FROM books ORDER BY id ASC`
	mock.ExpectQuery(regexp.QuoteMeta(pickerQuery)).
		WillReturnRows(sqlmock.NewRows([]string{"id", "title", "author", "supplier_id", "category", "condition", "mrp", "my_price", "bundle_price"}).
			AddRow(10, "Book A", "Author A", 1, "Fiction", "Very good", 400.0, 250.0, nil))
...
func TestPostgresStoreCreateAndGet(t *testing.T) {
	...
	getBundleQuery := `SELECT b.id, b.name, b.supplier_id, s.name AS supplier_name, b.category, array_to_string(b.allowed_conditions, '||') AS allowed_conditions, b.bundle_price, b.notes FROM bundles b JOIN suppliers s ON s.id = b.supplier_id WHERE b.id = $1`
	getBooksQuery := `SELECT bb.book_id, b.title, b.author, b.supplier_id, b.category, b.condition, b.mrp, b.my_price, b.bundle_price FROM bundle_books bb JOIN books b ON b.id = bb.book_id WHERE bb.bundle_id = $1 ORDER BY bb.position ASC`
	...
	mock.ExpectQuery(regexp.QuoteMeta(getBundleQuery)).WithArgs(22).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "supplier_id", "supplier_name", "category", "allowed_conditions", "bundle_price", "notes"}).
			AddRow(22, input.Name, input.SupplierID, "Supplier A", input.Category, "Very good||Good as new", input.BundlePrice, input.Notes))
	mock.ExpectQuery(regexp.QuoteMeta(getBooksQuery)).WithArgs(22).
		WillReturnRows(sqlmock.NewRows([]string{"book_id", "title", "author", "supplier_id", "category", "condition", "mrp", "my_price", "bundle_price"}).
			AddRow(10, "Book A", "Author A", 1, "Fiction", "Very good", 400.0, 250.0, nil).
			AddRow(11, "Book B", "Author B", 1, "Fiction", "Good as new", 500.0, 300.0, nil))
...
func TestPostgresStoreUpdateAndNotFound(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(getBundleQuery)).WithArgs(9).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "supplier_id", "supplier_name", "category", "allowed_conditions", "bundle_price", "notes"}).
			AddRow(9, updateInput.Name, updateInput.SupplierID, "Supplier 2", updateInput.Category, "Used", updateInput.BundlePrice, updateInput.Notes))
	mock.ExpectQuery(regexp.QuoteMeta(getBooksQuery)).WithArgs(9).
		WillReturnRows(sqlmock.NewRows([]string{"book_id", "title", "author", "supplier_id", "category", "condition", "mrp", "my_price", "bundle_price"}).
			AddRow(40, "B1", "A1", 2, "Non-Fiction", "Used", 300.0, 200.0, nil).
			AddRow(41, "B2", "A2", 2, "Non-Fiction", "Used", 250.0, 150.0, nil))
